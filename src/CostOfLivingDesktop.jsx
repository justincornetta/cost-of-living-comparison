...
export default function FinancialComparisonToolDesktop() {
  ...
  const s = {
    ...
    projCards: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 },
    ...
    tableHead: { display: 'grid', gridTemplateColumns: '80px repeat(3, 1fr)', background: '#F8FAFC', borderBottom: '2px solid #E2E8F0' },
    tableRow: { display: 'grid', gridTemplateColumns: '80px repeat(3, 1fr)', borderBottom: '1px solid #F1F5F9' },
          ...
          <section style={s.panel}>
            ...
            <div style={s.chart}>
              <svg style={s.chartSvg} viewBox="0 0 100 50" preserveAspectRatio="none">
                {['nyc', 'nj', 'philly'].map(loc => {
                  ...
                  return <polyline key={loc} points={pts} fill="none" stroke={locConfig[loc].color} strokeWidth="0.8" vectorEffect="non-scaling-stroke" />;
                })}
              </svg>
              <div style={s.legend}>
                {['nyc', 'nj', 'philly'].map(loc => (
            ...
            <div style={s.table}>
              <div style={s.tableHead}>
                ...
                {['nyc', 'nj', 'philly'].map(loc => <div key={loc} style={{ ...s.tableCell, color: locConfig[loc].color }}>{locConfig[loc].emoji} {locConfig[loc].name}</div>)}
              </div>
              {[0, Math.floor(proj.projectionYears / 4), Math.floor(proj.projectionYears / 2), Math.floor(proj.projectionYears * 3 / 4), proj.projectionYears].filter((v, i, a) => a.indexOf(v) === i).map(y => (
                <div key={y} style={{ ...s.tableRow, background: y === proj.projectionYears ? '#F0FDF4' : 'transparent' }}>
                  ...
                  {['nyc', 'nj', 'philly'].map(loc => <div key={loc} style={s.tableCell}>{fmtK(projections[loc].data[y]?.netWorth || 0)}</div>)}
                </div>
              ))}
            </div>
          </section>
